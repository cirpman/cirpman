...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (storedToken && userId) {
      setUser({ uid: userId });
      // Try to load profile from worker. If missing, clear stale local auth values
      // and redirect to auth page. Log details for debugging but avoid printing tokens.
      worker.post('/get-user-profile', { userId })
        ...
        .catch((err: any) => {
          ...
          // Clear stored auth to prevent repeated failing requests
          try {
          ...
          setProfile(null);
          setLoading(false);
          // If the worker returned a 404, it's a missing profile — redirect to sign-in/signup.
