...
const BlogPost = () => {
  ...
  const fetchPost = async () => {
    setLoading(true);
    try {
      // Fetch all posts from worker and find by slug (worker stores minimal fields)
      const response = await worker.post('/get-blog-posts', {});
      if (!response.ok) throw new Error('Failed to fetch blog post');
      const rows = await response.json();
      const posts = Array.isArray(rows) ? rows : rows.results || [];
      const normalized = posts.map((r: any) => ({
        id: r.id,
        title: r.title,
        slug: (r.slug || r.title || '').toLowerCase().replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-'),
        content: r.content || '',
        excerpt: r.excerpt || '',
        ...
        tags: r.tags ? (Array.isArray(r.tags) ? r.tags : JSON.parse(r.tags || '[]')) : [],
        meta_description: r.meta_description || '',
        view_count: r.view_count || 0,
        author: r.author ? { full_name: r.author } : { full_name: 'Unknown' }
      }));

      const postData = normalized.find((p: any) => p.slug === slug);
      if (!postData) throw new Error('Blog post not found');
