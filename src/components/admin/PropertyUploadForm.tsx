...
const PropertyUploadForm: React.FC<PropertyUploadFormProps> = ({ onSuccess, onCancel, property }) => {
  ...
  const uploadFile = async (file: File) => {
    try {
      // Read file as base64 and send to worker's /upload endpoint which
      // performs a server-side env.BUCKET.put — this avoids presigned URL
      // S3-signing issues and keeps upload logic consistent with gallery flow.
