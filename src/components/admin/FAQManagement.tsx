...
const FAQManagement = () => {
  ...
  const filteredFAQs = faqs.filter(faq => {
    ...
    const matchesSearch = q.includes(searchTerm.toLowerCase()) || a.includes(searchTerm.toLowerCase());
    const matchesCategory = categoryFilter === 'all' || faq.category === categoryFilter;
    return matchesSearch && matchesCategory;
  });

  const categories = [...new Set(faqs.map(faq => faq.category || 'uncategorized'))];
            ...
            {Object.entries(groupedFAQs).map(([category, categoryFAQs]) => (
              <div key={category}>
                <h3 className="text-lg font-semibold text-gray-900 mb-4 capitalize">
                  {category ? (category.charAt(0).toUpperCase() + category.slice(1)) : 'Uncategorized'} ({categoryFAQs.length})
                </h3>
